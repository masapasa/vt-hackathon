...
def create_canvas_and_annotation(manifest, image_url, canvas_id, label, anno_id, anno_page_id, cluster_data):
    ...
    # Extract page_image from cluster data
    for hit in cluster_data['hits']['hits']:
        if 'page_image' in hit['_source']:
            page_image_url = hit['_source']['page_image']
            break
...
cluster_data = {
    "hits": {
        "hits": [
            {
                "_source": {
                    "page_image": "https://chroniclingamerica.loc.gov/iiif/2/service%2Fndnp%2Fme%2Fbatch_me_camden_ver01%2Fdata%2Fsn78000873%2F00279524688%2F1878110701%2F0198.jp2/pct:4.509507,83.684080,11.277974,1.872369/full/0/default.jpg"
                }
            },
            {
                "_source": {
                    # This hit doesn't have a page_image
                }
            }
        ]
    }
...
canvas, anno = create_canvas_and_annotation(manifest, image_url, canvas_id, label, anno_id, anno_page_id, cluster_data)
